...
let solve_mat_rec primed matrix unprimed add constr print = 
  ...
  let add_vec_op_calc = Array.map (fun x -> Expr_to_opcalc.expr_to_opCalc (Expr_transforms.algebraic_expand (Expr_simplifications.automatic_simplify x)) ivar_ident) add in
